...
//  USB_OTG_BSP_Init(pdev);  
  ...
  /* Enable Interrupts */
  USB_OTG_BSP_EnableInterruptH(pdev);
}
...
void USBH_Process(USB_OTG_CORE_HANDLE *pdev , USBH_HOST *phost)
{
  ...
  if (((HCD_IsDeviceConnected(pdev) == 0)|| (HCD_IsPortEnabled(pdev) == 0))&& (phost->gState != HOST_IDLE)) 
  {
    if(phost->gState != HOST_DEV_DISCONNECTED) 
    {
      phost->gState = HOST_DEV_DISCONNECTED;
			...
			printf("HOST_DEV_DISCONNECTED aa!\r\n");   //debug
    }
  }
  ...
  case HOST_IDLE :
    ...
    {
      printf("HCD_IsDeviceConnected cc!\r\n");   //debug
			phost->gState = HOST_WAIT_PRT_ENABLED; 
      
      /*wait denounce delay */
      USB_OTG_BSP_mDelay(300);
      
      /* Apply a port RESET */
      HCD_ResetPort(pdev);      //han 延长reset 后delay的时间，提高重新插入后检测成功率，
  ...
  case HOST_ENUMERATION:  
    ...
    { 
      ...
      phost->gState  = HOST_SUSPENDED;
			printf("HOST_SUSPENDED aa!\r\n");
#else
      phost->gState  = HOST_USR_INPUT;
			printf("HOST_USR_INPUT aa!\r\n");
