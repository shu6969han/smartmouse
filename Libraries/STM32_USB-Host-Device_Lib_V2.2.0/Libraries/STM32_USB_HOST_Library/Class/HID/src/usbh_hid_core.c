...
{	
   //USBH_HOST *pphost = phost;
//    printf("USBH_HID_InterfaceDeInit\r\n");    //debug
...
{   
  ...
  case HID_REQ_GET_REPORT_DESC:
    ...
    {
      HID_Machine.ctl_state = HHID_REQ_SET_IDLE;
    }
    
    break;
    
  case HHID_REQ_SET_IDLE:
    
 //   classReqStatus = USBH_Set_Idle (pdev, pphost, 100, 0);//这里duration官方设置的是0,修改为100,提高兼容性	 
      classReqStatus = USBH_Set_Idle (pdev, pphost, 0, 0);//这里duration官方设置的是0,修改为100,提高兼容性	 
  
    /* set Idle */
    if (classReqStatus == USBH_OK)
    {
      HID_Machine.ctl_state = HHID_REQ_SET_PROTOCOL;  
    }
                                  ...
                                  uint8_t reportId)
{
  ...
  phost->Control.setup.b.wLength.w = 0;	//官方的这里设置的是0,导致部分鼠标无法识别,这里修改为100以后,识别率明显提高.
 // phost->Control.setup.b.wLength.w = 0;	//官方的这里设置的是0,导致部分鼠标无法识别,这里修改为100以后,识别率明显提高.
